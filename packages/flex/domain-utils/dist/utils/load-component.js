...
const fetchRemote = (parsedUrl, remoteName, nonce) => new Promise((resolve, reject) => {
...
export const loadComponent = async ({ path, remoteUrl, scope = 'default', nonce = '---CSP_NONCE---', apiUrl = null }) => {
    const [remoteName, moduleName] = path.split('/');
    const initRemote = async (nonce) => {
        ...
        if (!(remoteName in window)) {
            ...
            const parsedUrl = parseRemoteUrl(remoteUrl, remoteName);
            // Fetch the remote app. We assume our remote app is exposing a `remoteEntry.js` file.
            await fetchRemote(`${parsedUrl}/remoteEntry.js`, remoteName, nonce);
        }
    ...
    //   } else {
    //     return initRemote(generateFlexCSPNonce());
    //   }
    // } else {
    //   return initRemote(nonce);
    // }
