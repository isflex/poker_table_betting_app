...
FROM base-env AS flexiness-domain-content
...
ARG FLEX_PROJ_DIR_GENERATED=packages/flex/domain-content
ARG CI=${FLEX_CI}
...
--mount=type=cache,target=${FLEX_APP}/${FLEX_PROJ_DIR_GENERATED}/.yarn/.cache/webpack,id=flexiness_domain_content_cache,gid=1000,uid=1000 \
source ~/.profile && \
yarn nx run @flexiness/domain-content:build
