...
FROM base-env AS flexiness-domain-utils
...
ARG FLEX_PROJ_DIR_GENERATED=packages/flex/domain-utils
ARG CI=${FLEX_CI}
...
--mount=type=cache,target=${FLEX_APP}/${FLEX_PROJ_DIR_GENERATED}/.yarn/.cache/webpack,id=flexiness_domain_utils_cache,gid=1000,uid=1000 \
source ~/.profile && \
yarn nx run @flexiness/domain-utils:build
